...
export class Node extends React.Component<Props> {
  ...
  getRenderable() {
    let { node }: { node: any } = this.props
    node = Immutable.Map(node)
    console.log(node);
    const update: any = () => {
      // console.log('');
    }
    ...
    const tagEventHandlers = {
      onMouseDown: (e: any) => update(e, this, 'triggerSelect', { tailed: false })
    }
    ...
    const toggleExpand = (e: any) => {
      // (e: any) => update(e, this, 'toggleExpand')
    }
    const triggerSelect = ()=>{
      // (e) => update(e, this, 'triggerSelect', { tailed: true })
    }
    ...
    const head = (
      <div className={classnames(["Node__Tag", "Node__Head", modifier])} style={base} {...tagEventHandlers}>
        {name !== 'html' && (
          <span className="Node__Expander" style={baseExpander} onMouseDown={toggleExpand}>
      ...
      <div className="Node">
        ...
        {expanded && (
          <div className="Node__Children">
            {children.map((child: any, i: number) => <Node {...this.props} node={child} key={i} />)}
          </div>
        )}
        {expanded && (
          <div
  ...
  getOpenTag(selfclosing?: boolean) {
    ...
    return (
      <span className="Node__Wrap">
        ...
        {/* {attribs && attribs.entrySeq().map((key: any, value: any) => {
